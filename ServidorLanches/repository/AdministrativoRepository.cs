...
namespace ServidorLanches.repository
{
    ...
    WHERE id = @id";
        ...
        public List<CupomDesconto> GetCuponsDesconto()
        {
            ...
            string sql = "SELECT * FROM cupons_desconto";
            ...
            while (reader.Read())
            {
                lista.Add(new CupomDesconto
                {
                    Id = reader.GetInt32("id"),
                    Nome = reader.GetString("nome"),
                    Percentual = reader.IsDBNull("percentual") ? null : reader.GetDecimal("percentual"),
                    ValorFixo = reader.IsDBNull("valor_fixo") ? null : reader.GetDecimal("valor_fixo"),
                    DataValidade = reader.IsDBNull("data_validade") ? null : reader.GetDateTime("data_validade"),
                    Ativo = reader.GetBoolean("ativo")
                });
            }
        ...
        (nome, percentual, valor_fixo, data_validade, ativo)
        VALUES
        (@nome, @percentual, @valor_fixo, @data_validade, @ativo)";

            using var cmd = new MySqlCommand(sql, conn);

            cmd.Parameters.AddWithValue("@nome", cupom.Nome);
            cmd.Parameters.AddWithValue("@percentual", cupom.Percentual ?? (object)DBNull.Value);
            cmd.Parameters.AddWithValue("@valor_fixo", cupom.ValorFixo ?? (object)DBNull.Value);
            cmd.Parameters.AddWithValue("@data_validade", cupom.DataValidade ?? (object)DBNull.Value);
            cmd.Parameters.AddWithValue("@ativo", cupom.Ativo);
