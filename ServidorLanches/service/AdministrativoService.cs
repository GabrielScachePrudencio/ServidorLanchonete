...
namespace ServidorLanches.service
{
    public class AdministrativoService
    {
        ...
        public List<CupomDesconto> GetAllCupons()
        {
            return administrativoRepository.GetCuponsDesconto();
        }

        public bool AddCupom(CupomDesconto cupom)
        {
            if (string.IsNullOrWhiteSpace(cupom.Nome))
                return false;

            return administrativoRepository.AddCupomDesconto(cupom);
        }

        public bool UpdateCupom(int id, CupomDesconto cupom)
        {
            if (id <= 0 || string.IsNullOrWhiteSpace(cupom.Nome))
                return false;

            return administrativoRepository.UpdateCupomDesconto(id, cupom);
        }

        public bool DeleteCupom(int id)
        {
            if (id <= 0)
                return false;

            return administrativoRepository.DeleteCupomDesconto(id);
        }
    }
}
